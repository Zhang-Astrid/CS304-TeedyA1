...
import com.sismics.docs.core.service.FileTranslationService;
...
public class DocumentResource extends BaseResource {
    ...
    // 常量提取
    private static final String MSG_NO_FILE = "No file found for document";
    private static final String MSG_INVALID_CONTENT_TYPE = "Invalid contentType";
    private static final String MSG_UNSUPPORTED_FILE_TYPE = "Unsupported file type: ";
    private static final String MSG_FILE_CREATOR_NOT_FOUND = "File creator not found";
    private static final String MSG_FAILED_DECRYPT = "Failed to decrypt file: ";
    private static final String MSG_ERROR_PROCESS_FILE = "Error processing file: ";
    private static final String MSG_NO_TEXT = "No text to translate";
    private static final String MSG_TRANSLATION_ERROR = "Error translating document: ";
    ...
    // 权限校验提取
    private void checkWritePermission(String docId, List<String> targetIds) {
        AclDao aclDao = new AclDao();
        if (!aclDao.checkPermission(docId, PermType.WRITE, targetIds)) {
            throw new ForbiddenClientException();
        }
    }

    // 参数校验提取
    private void validateNotEmpty(String value, String errorMsg) {
        if (value == null || value.trim().isEmpty()) {
            throw new ClientException("ValidationError", errorMsg);
        }
    }

    // 文件文本提取逻辑提取
    private String extractTextFromDocumentFile(String docId, String targetLanguage) {
        FileDao fileDao = new FileDao();
        List<File> files = fileDao.getByDocumentId(null, docId);
        if (files == null || files.isEmpty()) {
            log.error("[extractTextFromDocumentFile] No file found for document");
            return null;
        }
        File file = files.get(0);
        String filePath = file.getPath();
        log.info("[extractTextFromDocumentFile] filePath: {}", filePath);

        try {
            java.nio.file.Path storedFile = java.nio.file.Paths.get(filePath);
            String mimeType = file.getMimeType();
            log.info("[extractTextFromDocumentFile] File MIME type from database: {}", mimeType);

            String detectedFileType = getFileTypeFromMimeType(mimeType);
            if (detectedFileType == null) {
                log.error("[extractTextFromDocumentFile] Unsupported file type: {}", mimeType);
                return null;
            }
            ...
            try {
                ...
                if (creator == null) {
                    log.error("[extractTextFromDocumentFile] File creator not found");
                    return null;
                }
                java.nio.file.Path unencryptedFile = EncryptionUtil.decryptFile(storedFile, creator.getPrivateKey());
                decryptedFileBytes = java.nio.file.Files.readAllBytes(unencryptedFile);
                log.info("[extractTextFromDocumentFile] Successfully decrypted file content");
            } catch (Exception e) {
                log.error("[extractTextFromDocumentFile] Error decrypting file: {}", e.getMessage());
                return null;
            }

            return processFileWithOcrSpace(decryptedFileBytes, detectedFileType, targetLanguage);
        } catch (Exception e) {
            log.error("[extractTextFromDocumentFile] Error processing file: {}", e.getMessage());
            return null;
        }
    }
    ...
    public Response translate(
        ...
        if (doc == null) {
            return buildErrorResponse(Response.Status.NOT_FOUND, MSG_NO_FILE);
        }
        ...
        try {
            ...
            if ("description".equals(contentType)) {
                sourceText = doc.getDescription();
            } else if ("file".equals(contentType)) {
                sourceText = extractTextFromDocumentFile(id, targetLanguage);
                if (sourceText == null) {
                    return buildErrorResponse(Response.Status.BAD_REQUEST, MSG_NO_FILE);
                }
            } else {
                log.error("[translate] Invalid contentType: {}", contentType);
                return buildErrorResponse(Response.Status.BAD_REQUEST, MSG_INVALID_CONTENT_TYPE);
            }

            if (sourceText == null || sourceText.trim().isEmpty()) {
                log.error("[translate] No text to translate");
                return buildErrorResponse(Response.Status.BAD_REQUEST, MSG_NO_TEXT);
            }

            String translatedText;
            try {
                translatedText = translateWithBaidu(sourceText, targetLanguage);
                ...
                return buildErrorResponse(Response.Status.INTERNAL_SERVER_ERROR, MSG_TRANSLATION_ERROR + e.getMessage());
            }

            JsonObjectBuilder jsonResponse = Json.createObjectBuilder();
            jsonResponse.add("translated", translatedText);
    ...
    private String extractTextFromOfficeDocument(byte[] fileData, String fileType) throws IOException {
        try {
            if (fileType.equalsIgnoreCase("docx")) {
                try (XWPFDocument doc = new XWPFDocument(new java.io.ByteArrayInputStream(fileData))) {
